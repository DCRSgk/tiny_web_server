...
namespace base
{
    ...
    public:
        ...
        bool Error3(const char* file, int line, const char* format, ...);
...
#define LOG_DEBUG_C(name, format, ...) \
if (model::Module_ServerConfig::Instance()->logSwitch().name) \
    base::FileLogger::instance().Debug(format, ##__VA_ARGS__);
    
#define LOG_WARN_C(name, format, ...) \
if (model::Module_ServerConfig::Instance()->logSwitch().name) \
    base::FileLogger::instance().Warn(format, ##__VA_ARGS__);
    
#define LOG_ERROR_C(name, format, ...) \
if (model::Module_ServerConfig::Instance()->logSwitch().name) \
    base::FileLogger::instance().Error(format, ##__VA_ARGS__);

#define LOG_ASSERT(condition) if (!(condition)) { base::FileLogger::instance().Error3(__FILE__, __LINE__, "asset fail"); return; }
#define LOG_CHECK(condition, format, ...) (!(condition) && \
base::FileLogger::instance().Error3(__FILE__, __LINE__, format, ##__VA_ARGS__))

#define LOG_CHECK_RETURN(retVal, condition, format, ...) do {\
    if (!(condition)) {\
        base::FileLogger::instance().Error3(__FILE__, __LINE__, format, ##__VA_ARGS__);\
        return retVal;\
    }\
...
#define LOG_PANIC() base::FileLogger::instance().Error3(__FILE__, __LINE__, "panic")
