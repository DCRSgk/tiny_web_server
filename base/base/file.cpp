...
namespace base
{
    namespace utils
    {
        ...
        static void write_all(FILE* f, const char* buffer, size_t bufferLen)
        {
            size_t offset = 0u;
            while (offset < bufferLen) {
                int eated = fwrite(buffer + offset, 1, bufferLen - offset, f);
                if (eated == EOF) {
                    break;
                }
                offset += eated;
        ...
        bool file_save_content(const char* filename, const string& content)
        {
            ...
            if (f) {
                write_all(f, content.c_str(), content.length());
                fflush(f);
                fclose(f);
                return true;
            }
        ...
        bool file_save_buffer(const char* filename, const char* data, size_t dataLen)
        {
            FILE* f = fopen(filename, "w+");
            if (f) {
                write_all(f, data, dataLen);
                fflush(f);
                fclose(f);
                return true;
            }
        ...
        bool file_append_content(const char* filename, const string& content)
        {
            ...
            if (f) {
                write_all(f, content.c_str(), content.size());
                fflush(f);
                fclose(f);
                return true;
            }
